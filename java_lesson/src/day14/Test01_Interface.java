...
/*
	...
	==> 클래스의 한종류이긴 하지만
		...
		1. 최상위 클래스가 Object 클래스가 아니다.
			==> 인터페이스는 다른 클래스를 상속받아서 만들어진 클래스가 아니다.
...
----------------------------------------------------------------------------------------------
	...
	참고 ]
		인터페이스를 상속 받아서 인터페이스를 만들 수 있다.
		
		형식 ]
			interface 상속받을인터페이스이름 extends 상속해줄인터페이스 {
				물러받은 추상함수
				+
				내가만든 추상함수
			}
...
public class Test01_Interface {
	public Test01_Interface() {
		// Figure를 상속받은 클래스 Nemo, Semo 10개를 기억할 배열 fig 만들고 랜덤하게 채워보자.
		...
		for(int i = 0 ; i < 10 ; i++ ) {
			...
			// 아직 Nemo의 area가 셋팅이 안됬으므로 셋팅해준다.
			fig[i].setArea();
		}
