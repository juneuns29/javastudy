...
/*
	Set 계열
			...
			생성자 ]
				...
				==> 
					float loadFactor : 해쉬코드값을 만들 규칙에 사용할 상수값
										사용자가 지정하면서 해쉬테이블을 만들어서 사용
				
			
		2. TreeSet
		==> 내부적으로 Tree Sort 라는 알고리즘을 이용해서 데이터를 정렬해서 보관
			우리가 알고 있는 정렬방식을 사용한다.
			
			참고 ]
				TreeSet에는 정렬되지 않는 데이터는 입력할 수 없다.
		...
		참고함수 ]
			subSet()
			==> 하나의 Set에 담긴 내용 중에서 특정 부분만 골라서 새로운 Set 을 만드는 방법
 ...
 */
	public Test02_Set() {
		...
		// 꺼내는 함수가 없기때문에 순차적으로 꺼내는 타입으로 변환시켜야 된다.
		// 방법 1.
		Iterator itor = set.iterator();
		// Iterator - 버퍼메모리에 기억되고 꺼내는 순간 삭제하는 타입
		while(itor.hasNext()) {
			Object o = itor.next(); // 순차적으로 데이터 꺼낸다.
