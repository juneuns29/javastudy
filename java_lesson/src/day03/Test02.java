...
public class Test02 {
	/*
		키보드를 이용해서 데이터 입력받기
			...
			1. 클래스를 만들기 전에 
				import java.util.*;
			라고 java폴더내의 util 폴더 안의 모든 클래스를 사용할 준비를 한다.
			...
			* 여기서는 문자열로 숫자를 입력받아서 
				문자열을 정수로 변환해서 
				연산을 하고 그 결과를 출력해보기로 하자.
	 */
	
	/*
		숫자형태의 문자열을 숫자로 변환하는 방법
			
			1. 정수
				int no = Integer.parseInt(정수 형태의 문자열);
			2. 실수
				double no1 = Double.parseDouble(실수형태의 문자열);
	 */
	...
	public static void main(String[] args) {
		...
		sno1 = sc.nextLine(); // 입력된 내용을 문자열로 읽어오는 함수
		System.out.print("세로 입력 : ");
		sno2 = sc.nextLine();
		// 숫자형태의 문자열을 정수로 변환해준다.
		int garo = Integer.parseInt(sno1);
		int sero = Integer.parseInt(sno2);
		
		// 정수로 변환된 가로, 세로를 이용해서 면적 계산
		int area = garo * sero;
		...
		/*
			참고 ]
				...
				자바에서는 + 를 String 데이터에서는 
				결합연산자로 사용할 수 있도록 해놓은 것이다.
				따라서 다른 클래스 타입의 데이터에서는 + 연산자를 사용할 수 없다.
				그리고 + 이외의 다른 연산자( - , *, /, %)는 
				문자열에서 정의 해놓지 않았기 때문에 사용할 수 없다.
