...
public class FormatClass {
		...
		==> 데이터를 형식에 맞춰서 출력하기 위한 목적으로 사용되어지는 클래스
			...
			==> 숫자를 형식을 지정해서 문자열로 만들어주는 클래스
				...
				생성방법 ]
					...
					==>
						pattern : 숫자를 어떤형태의 문자열로 만들것이지 형식을 지정
						
				주요함수 ]
					format(double number)
					==> 숫자를 형식에 맞춰서 문자열로 변환시켜주는 함수
					
				참고 ]
					패턴 지정방법
						0		- 한자리 숫자를 의미하고 무효숫자도 표현
						#		- 한자리 숫자를 의미하고 무효숫자는 표현하지 않는다.
						.		- 소수점 위치
						,		- 3자리마다 , 표시하도록
						-		- 음수일 경우 - 부호의 위치
						E		- 실수의 경우 지수 형태로 표현
			...
			2. SimpleDateFormat
			==> 날짜 데이터를 원하는 형식의 문자열로 변환해주는 클래스
	...
	public FormatClass() {
		...
		System.out.println("DecimalFormat : " + snum);
		...
		/*
			주의사항 ]
				...
				formats를 만들때는
				반드시 limits의 갯수와 동일한 갯수로 만들어야 한다.
		 */
		...
		int score = 1000;
		int luffy = 3500;
		System.out.println("쵸파 학점 : " + cForm.format(score));
		System.out.println("루피 범죄자 등급 : " + cForm.format(luffy));
		System.out.println();
		System.out.println("쵸파 범죄자등급 : " + cForm2.format(score));
		System.out.println("루피 범죄자 등급 : " + cForm2.format(luffy));
		System.out.println();
		...
		String jniInfo = mForm.format(jennie);
		System.out.println(jniInfo);
		Object[][] data = {
				{"제니", 28},	
				{"리사", 26},	
				{"로제", 27},	
