	...
	==>  문자열을 관리하는 클래스
		...
		주의 ]
			...
			==> String 클래스는 내용이 변경이 되면
				...
				결론적으로 변환가 심한 문자열은 String 클래스를 이용하면 안된다.
 ...
 ---------------------------------------------------------------------------------------------------------
 	...
 	==> String 클래스와 동일하게 문자열을 관리하는 클래스이다.
 		...
 		차이점 ]
 			...
 			StringBuffer의 경우는 기존 데이터를 수정해서 보관한다.
 		...
 		참고 ]
 			실무에서 StringBuffer 사용방법
 				...
 				1. String 이 편하므로 먼저 String 타입으로 변수를 만든다.
 					...
 					String str = "jennie";
 				2. 문자열의 변환가 필요한 시점에 StringBuffer로 변환해서
 	...
 	==> 주로 수학적인 기능을 처리하는 함수로 구성된 클래스
 		멤버들이 모두 static 이므로 별도로 new 시키지 않아도 사용할 수 있다.
 		그리고 아예 new 시키지 못하도록 막아놓은 클래스.....
 		<== 기본생성자의 접근지정자를 private로 정의해놓아서
 			다른 클래스에서는 new 시키지 못하도록 조치해놓은 클래스이다.
 			
 	---------------------------------------------------------------------------------------------------
 	Wrapper Class
 	==> 참조형 타입의 변수에는 기본형 타입의 데이터를 사용하지 못하는 문제가 생길 수 있다.
 		이런 문제를 해결하기 위해서 만든 클래스들을 래퍼클래스(Wrapper Class)라고 부른다.
 		즉, int 형태의 데이터는 참조형이 아니므로 객체가 아니고 
 		이렇다보니 객체형태로 사용해야될 필요가 있을경우 문제가 생길 수 있다.
 		이런 문제를 해결하기 위해서 만들어진 클래스들이다.
 		
 		자바는 Object 클래스를 중심으로 계층구조화해서
 		클래스를 만들도록 해놓은 객체지향 언어이다.
 		그런데 기본데이터타입의 데이터의 경우는 클래스가 아니기 때문에 계층구조에서 제외가 되는 문제가 생긴다.
 		...
 		참고 ]
 			...
 			==> Value Type 을 주소타입으로 변환하는 것
 				(기본형변수를 참조형 변수로 변환하는 작업)
 			UnBoxing
 			==> 주소타입을 다시 Value Type으로 변환하는 것.
 				(참조형타입을 기본형타입으로 변환하는 작업)
 				
 			래퍼클래스는 Boxing과 UnBoxing을 해주기 위한 클래스
 			
 		참고 ]
 			그런데 위 내용은 몰라도 됩니다.
 			자바 버젼 1.4부터 Boxing, UnBoxing이 자동으로 처리되기 때문이다.
