...
/*
	*****
	...
	==> 원칙적으로 주소는 다른 형태로 형 변환이 불가능하다.
		...
		***
		...
		<== is a 관계이기 때문에...
			...
			강제형변환
			==> 상위 클래스의 주소가 하위클래스의 변수에 기억되는 것은 
				원칙적으로 불가능하지만
				강제로 형변환은 가능하다.
	...
	참고 ]
		상속받은 클래스에서 상위 클래스의 함수를 오버라이딩하는 경우
		하위클래스의 인스턴스를 상위 클래스의 변수에 기억시켜서 사용할 때
		상위클래스의 함수를 호출할 경우 하위클래스의 오버로딩된 함수가 호출된다.
		
참고 ]
	어떤 함수의 반환값이 상위 클래스 형태로 되어있으면
	그 반환값을 사용할 때는 반드시 하위 클래스 형태로 형변환해서 사용해야 한다.
...
//		Son s = (Son)(new Father()); // 강제형변환
		...
		System.out.println((int) abc(10) + 10);
