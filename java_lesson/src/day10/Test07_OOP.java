...
/*
	...
	*****
	그런데 클래스는 설계도에 불과하다.
	즉, 클래스만 가지고 있다고 이것이 실행되는 것은 절대아니다.
	반드시 클래스가 실행가능한 것으로 탄생되어야 한다.
	(new 명령을 이용해서 Heap 영역에 올려놓는 작업...) 
	
	이렇게 클래스를 실행가능한 것으로 만든 것을 "Object(객체)" 라고 부른다.
	...
	객체 사용방법
		==> 객체를 사용한다는 것은
			...
			형식 ]
				...
				참고 ]
					자바에서는 소속을 밝혀줄때 "." 을 이용한다.
				
	참고 ]
		클래스 하나당 오브젝트(객체)는 복수개 만들어도 상관없다.
		즉, 설계도 한장만 있으면 집 여러개 지어도 상관없다.
		이때 주소를 이용해서 사용하게 되므로
		각가의 오브젝트는 별개로 독립되어서 실행될 수 있다.
