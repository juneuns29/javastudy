...
/*
	IO (Input/Output : 외부 장치와의 통신에 필요한 것들을 모아놓은 패키지)
	
		스트림(Stream)
		==> 자바는 외부 장치와 데이터를 주고 받는 방법을 
			하나의 방법으로 통일 시켜 놓았다.
			그 방법이 Stream 이다.
			
			스트림은 빨대와 같다.
			
			즉, 스트림을 데이터를 주고 받을 외부 장치에 꽂기만 하면
			나는 빨대를 흡입만 하면(같은 함수를 이용한다.)
			외부장치의 데이터가 들어온다.
			
			참고 ]
				스트림은 단방향이다.
				즉, 하나의 스트림은 한방향으로만 데이터가 전송된다.
	...
	스트림의 종류
		...
		데이터의 양에 대한 분류
			1. byte 단위 : 속도는 1바이트씩 처리하므로 조금 느리다.
							기계어도 처리가 가능하다.
							==> 실행파일도 전송가능하다.
							
				InputStream, OutputStream
				
			2. 문자 단위(char 단위 , 2byte) : 속도는 조금 빠르다. 오직 문자만 처리가능하다.
			
				Reader, Writer
				
			참고 ]
				byte가 char로 합쳐지는 순간
				운영체제에 따라서 비트 순서가 뒤바뀔 수 있다.
				따라서 실행파일이나 이미지 파일 등은 문자단위로 전송하면 안된다.
				
				
		대상에 따른 분류
			1. 타겟스트림(기본 스트림, Target Stream)
				==> 외부장치에 직접 연결되는 스트림
					따라서 외부장치와 통신을 하려면
					이 기본스트림이 필수적이다.
					
			2. 필터스트림(보조스트림)
				==> 스트림의 성능향상, 기능향상, 편의성 제공 을 위해서
	...
	1. InputStream
		...
		주요함수 ]
			...
			int read(byte[] b)
			==> 여러 바이트를 읽어들이는 함수
				읽은 결과는 byte[] b 에 저장된다.
				
				반환값은 읽은 바이트 수를 반환해준다.
...
----------------------------------------------------------------------------------------------------------
	...
	==> 이것은 스트림은 아니다.
		(데이터를 입출력하는 기능은 없다.)
		다만 우리가 외부장치 중에서 가장 빈번하게 사용하는 외부장치가
		파일(파일도 외부 장치로 취급한다.)이다 보니 그 파일에 대한 정보를 처리하기 위한 클래스
		...
		생성방법 ]
			...
			==> 파일의 정보를 이용하고 파일의 이름을 다시 저정해서 만드는 방법
	...
	*****
	...
	2. DataInputStream / DataOutputStream
		==> 많이 사용하는 스트림은 아니다.
			...
			문제가 있을 수 있다.
			==> 자바의 데이터형을 byte[]변환 하지 않고
				직접 외부 장치에 입출력 가능한 기능을 가지고 있다.
				
			많이 사용하지 않는 이유 ]
				DataOutputStream으로 내보낸 데이터는 반드시 DataInputStream을 이용해서 읽어야 한다.
				그리고 꺼낼때 순서도 지켜야 한다.
				(저장할 때 int, float, char ==> 읽을때도 int, float, char 순서로 읽어야 한다.)
	...
	public Test01_IO() {
		...
		try {
			...
			*/
			...
			//		1. 여러글자를 기억할 배열(byte[])을 준비
			byte[] buff = new byte[256];
			// 256은 한번에 읽을 수 있는 문자의 갯수를 의미한다.
			int len = System.in.read(buff);
			// len : 읽은 바이트 수
			// 읽은 결과를 출력
			// String(byte[] bytes, int offset, int length) 
			// ==> 바이트 배열을 입력하고 꺼낼위치와 갯수를 입력해주면서 문자열 만드는 방법
...
//			System.out.println(result);
			...
			// 준비된 배열인 buff에 10번 방 부터 읽은 내용을 기억하세요.
