...
-------------------------------------------------------------------------------
	...
	==> 이미 만들어진 클래스의 기능을 그대로 부여받아서
		...
		그럴때 처음부터 다시 만들게 되면 개발속도가 저하될 것이다.
		이 문제를 해결하기 위해서 만든 방법이 상속이다.
		
		상속은 자신이 만들고자 하는 부품(클래스)과 가장 유사한 부품을 가지고 와서
			일단 그 기능을 모두 복사한 후
			1. 없는 기능은 덧붙여 넣고
			2. 있는 기능중 바꿀내용은 바꿔서 사용하자.
		
		상속을 받는 형식 ]
			
			[ 접근지정자 ]  [속성] class 클래스이름  extends 상속해주는클래스이름 {
				// 이 내부에는 상속해주는클래스의 내용이 모두 들어와 있게 된다.
			}
		
	참고 ]
		상속은 오직 단일 상속만 가능하다.
		( ==> 여러클래스로 부터 동시에 상속 받을 수 없다.)
...
---------------------------------------------------------------------------
	...
	상속 관계에 있는 경우 생성자의 역할
		...
		상위 클래스의 생성자 함수는 자동으로 호출되는 개념이다.
	...
	***
	참고 ]
		...
		특징 ]
			1. 반드시 생성자 함수내에서만 호출할 수 있다.
			2. 반드시 생성자 내의 첫행 첫 명령으로만 사용할 수 있다.
