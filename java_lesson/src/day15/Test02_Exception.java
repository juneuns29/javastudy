...
/*
	...
	==> 프로그램적으로는 문제가 없으나
		...
		문제는 
		이런 예외는 개발자 예측해서 
		이런 문제가 발생했을 경우 대처하는 방식으로 프로그램을 제작해야하고
		...
		참고 ]
			3) 번의 경우는 예외가 발생하지 않은 경우에만 실행된다.
			예외가 발생하면 실행되지 않는다.
			==> 따라서 try{} 의 범위에 따라서 
				실행될 수도 있고 
				실행이 안될 수도 있다.
			...
			이런 경우를 위해서
			예외의 발생여부와 관계없이 반드시 실행되어야 되는 명령은
			5) 위치에 기술해야 한다.
		
	자바의 예외처리 원리
		예외가 발생하면 JVM 은 발생한 예외의 종류를 분석한 후
...
//		System.out.println(1 / 0);
		...
		try {
			...
			// 여기서 변수 e 는 catch 블럭에서만 사용하는 지역변수
