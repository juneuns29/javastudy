	...
	2. 전역 Inner Class(Global Inner Class)
		==> 일반 클래스를 내부 클래스로 만든 것.
			...
			특징 ]
				...
				2. 반드시 바깥 클래스가 new 된 이후에 안쪽 전역 내부클래스를 new 시킬 수 있다.
					
					형식 ]
						바깥클래스주소.new 전역내부클래스();
						
				3. 바깥 클래스는 안쪽 전역내부클래스의 멤버를 사용할 수 없고
	...
	3. 지역 Inner Class(지역 내부 클래스)
		...
		***
		...
		==> 이름 그대로 이름이 없는 내부 클래스
			...
			이때 원칙적인 규칙은 
			상속 받은 클래스를 별도로 만들고 그 클래스를 new 시켜서 사용하는 것이
			원칙이다.
			
			이때 간단한 오버라이딩을 해야하는 경우
...
//		Test01_InnerClass.TopLev.abc();
		...
		// 전역내부 클래스
		GlobClass g1 = new GlobClass(); // 이클래스가 객체가 되어야 GlobClass를 new 시킬 수 있다.
		g1.ch = 'Z';
		g1.xyz();
	}
