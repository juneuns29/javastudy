...
/*
	...
	==> 자바에서 최상위 클래스
		...
		소속함수 
			...
			참고 ]
				String클래스가 가지고 있는 equals() 함수는 
				Object 클래스에서 물려받은 주소를 비교하는 equals() 함수를
				문자열 데이터를 비교하는 기능으로 고쳐서 오버라이드 한 함수이다.
				
				만약 우리가 어떤 클래스를 제작하고
				그 클래스이 내용이 같은지를 비교하도록 하려면
...
*/
		...
		Box b1 = new Box(4, 5); // 면적은 20.0
		Box b2 = new Box(2, 10); // 면적은 20.0
		Box b3 = new Box(3, 5); // 면적은 15.0
		...
		System.out.println("b1과 b2 는 [ " + ((b1.equals(b2) ? "같은" : "다른")) + " ] Box 입니다.");
		System.out.println("b1과 b3 는 [ " + ((b1.equals(b3) ? "같은" : "다른")) + " ] Box 입니다.");
	}
...
// Box 클래스를 만드는데 면적이 같으면 같은 클래스가 되도록 제작한다.
class Box {
	...
	public boolean equals(Object o) {
		/*
			...
			따라서 원래 형태로 강제 형변환해줘야 그 기능을 사용할 수 있게 된다.
		 */
		Box b = (Box) o; // 강제 형변환
