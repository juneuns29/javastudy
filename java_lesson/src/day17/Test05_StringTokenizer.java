...
public class Test05_StringTokenizer {
	...
	public Test05_StringTokenizer() {
		String str = "The string tokenizer class allows " + 
					"an application to break a string into tokens.";
		
		StringTokenizer token = new StringTokenizer(str);
		// 잘라진 단어를 배열에 저장해보자.
		// 배열은 만들때 저장할 형태와 갯수를 지정해줘야 만들 수 있다.
		int len = token.countTokens(); // 잘려진 갯수 반환해주는 함수
		String[] words = new String[len];
		int idx = 0;
		while(token.hasMoreTokens()) {
			// hasMoreTokens() : 꺼내올 문자열이 있는지를 반환해주는 함수(boolean)
			// hasMoreElements() : 꺼내올 오브젝트가 있는지를 반환해주는 함수(boolean)
