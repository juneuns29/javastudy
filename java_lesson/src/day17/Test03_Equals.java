...
class Stud implements Cloneable {
	...
	public Stud(int no, int kor, int eng, int math) {
		...
		// 이때 반드시 생성자의 첫문장, 첫 명령으로 사용해야 한다.
		this.kor = kor;
		this.eng = eng;
		this.math = math;
		setTotal();
	...
	public void setAvg() { // Overloading
		avg = total / 3.0;
	}
	...
	public Object clone() throws CloneNotSupportedException {
		/*
			...
			그러면 이 함수의 접근지정자를 public으로 고쳐쓰면 
			다른 곳에서도 사용가능해질 것이다.(오버라이딩 규칙 2번)
			
			문제는 오버라이딩을 하는 순간 상위클래스가 가졌던 기능은 사라지게 된다.
			그러면 이 함수 내에서 자기자신을 복제해주는 기능을 구현해줘야 하겠다.
			상속 받은 클래스에서 상위 클래스의 멤버를 강제로 사용하는 방법 하나 있다.
				super
			라는 약속된 변수를 사용하면 된다.
		 */
		Object o = super.clone(); // 상위 클래스의 기능으로 
