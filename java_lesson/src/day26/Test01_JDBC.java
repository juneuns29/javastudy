...
public class Test01_JDBC {
	...
	public Test01_JDBC() {
		try {
			setCon(); // 이 함수의 기능은 전역변수 con을 셋팅해주는 기능
...
//		System.out.println("SQL : " + sql);
		...
		// 질의명령이 완성됬으므로 질의명령을 보내고 결과를 받으면 된다.
		...
		if(row == 1) {
			...
			System.out.println("### 비밀번호 변경 작업에 문제가 있습니다. ###");
		}
...
//		System.out.println("*** SQL : " + sql);
		...
		// 질의명령이 준비됬으므로 이 질의명령을 전달할 도구가 필요하다.
		setStmt();
		
		// 전달 도구가 준비됬으므로 질의명령을 실어서 보내고
		// 결과를 받으면 된다.
		// 이때 결과는 JDBC가 SELECT 질의명령의 결과를 ResultSet 객체로 반환해준다.
		
		rs = stmt.executeQuery(sql);
		// 질의명령의 결과는 rs에 담겨있고
		// 이때 커서는(작업진행줄) BOF에 있다.
		// 그래서 데이터를 꺼내려면 BOF 다음의 첫행으로 작업진행줄을 이동시켜야 한다.
		// 그 명령이 rs.next() 이고 이 함수의 반환값은 
		// 		true : 이동이 성공한 경우(EOF 가 아닌경우)
		// 		false : 이동한 행이 EOF인 경우(꺼내올 데이터행으로 이동을 실패한 경우)
		
		// 조회결과가 몇행이 될지 알수 없으므로
		while(rs.next()) {
			// 작업진행줄을 한행 내리는 작업이 성공한 경우에만 반복을 진행
			String name = rs.getString("name"); 
			// 결과 행에서 name 컬럼의 데이터를 꺼내서 변수에 기억시킨다.
				...
				주의사항 ]
					...
					잘못된 예 ]
						...
						==> 이렇게 결합하게 되면 결과는
							...
							이런경우는 데이터베이스에서 오류가 발생할 것이다.
							따라서 결합할때 각 문자열 데이터 맨 뒤에 공백을 습관적으로
							추가해주도록 한다.
