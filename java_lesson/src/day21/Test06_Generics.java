...
public class Test06_Generics {
	...
	public Test06_Generics() {
		...
		// 이 리스트의 저장할 수 있는 데이터는 Object 데이터이고
		// 자바에서 모든 객체는 Object 타입으로 볼 수 있다.
		...
		// list1에 Object 형태로 저장되어있으므로
		// 꺼내서 사용할 때는 원래형태로 강제 형변환 해줘야 한다.
		...
		for(String s : list2) {
			// list2에는 문자열 데이터만 저장하도록 제너릭스 선언을 해뒀으므로
			// 꺼낸 데이터는 당연히 문자열일 것이다.
			// 따라서 강제 형변환이 필요없다.
