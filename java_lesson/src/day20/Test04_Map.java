...
/*
	...
	==>
		...
		==>
			Hashtable이 구버젼으로
			HashMap 새로운 버젼이므로 주로 HashMap을 권장한다.
			...
			참고 함수 ]
				...
				keys()
				==> 키값만 꺼내주는 함수
					Map은 키값을 알아야 데이터를 꺼낼 수 있기 때문에...
					
				참고 ]
					keys() 로 꺼낸 데이터는 Enumeration으로 반환된다.
					==> Enumeration 클래스는 Iterator의 구버젼이다.
						따라서 Iterator와 사용법이 비슷하고
						순차적으로 데이터를 꺼낼 수 있다.
		...
		참고 ]
			...
			==> Map은 반드시 키값을 알아야 데이터를 꺼낼 수 있다.
				이 함수는 키값과 데이터를  VO에 담아서 Set으로 반환해주는 함수다.
				
				Map은 항상 키값과 데이터가 한쌍이 되어야 입력할 수 있다.
				Set은 오직 한개만 저장한다.
				그래서 키값과 데이터를 기억시키는 VO를 만들어서 
				VO 에 키값과 데이터를 저장시킨후
				Set 에 저장해서 Set을 반환해준다.
				
	-------------------------------------------------------------------------------
	TreeMap
	==> TreeSet과 마찬가지로 내부에서 정렬하면서 데이터를 보관하는 Map
		
		이때 정렬할 때는 데이터를 이용해서 정렬하는 것이 아니고
		키값을 이용해서 정렬하게 된다.
 ...
 */
	public Test04_Map() {
		...
		// 키값들만 추출해서 모든 데이터 출력
		Enumeration en = map.keys(); // map에 기억된 키값들만 모두 꺼내주는 함수
		while(en.hasMoreElements()) {
			/*
				hasMoreElements() : 꺼낼 데이터가 있는 지 여부를 알려주는 함수
			 */
		...
		System.out.println();
		System.out.println("######### keySet() ==> ArrayList로 변환 ##########");
