	...
	public static void main(String[] args) {
		...
		*/
		...
		for(int i = 0 ; i < ch.length ; i++ ) {
			...
			// 카운트 만큼 별이 출력이 됬으니 줄바꿈 해준다.
			System.out.println();
		}
		*/
		
		// 두개를 한개의 배열에 저장해서 처리하는 방법
		int[][] arr = new int[2][10];
		
//		arr[0] = ch; // 힙타입이 달라서 에러...
		for(int i = 0 ; i < 10 ; i++ ) {
			arr[0][i] = 'a' + i;
		}
		...
		/*
			참고 ]
				위 배열에서 꺼내온 데이터들의 형태
					...
					arr[0] : arr 첫번째 방에 들어있는 데이터 열개를 기억하는 배열
					arr[0][0] : arr첫번째 방의 배열의 첫번째 방의 숫자
		 */
