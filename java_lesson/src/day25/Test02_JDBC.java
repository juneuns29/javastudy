...
/*
	...
	==> 자바 프로그램을 이용해서 데이터베이스를 컨트롤 할 수 있는 외부  API이다.
		...
		이 말은 각각의 데이터베이스마다 질의명령을 처리하는 방법(DBMS)이 다르다는 것이다.
		
		이것은 개발자 입장에서 보면 데이터베이스마다
		그 데이터베이스의 DBMS에 따라서 프로그램 방식을 다르게 해야한다는 것이 된다.
		
		따라서 해결방법으로
		자바 안에 DBMS을 만들어 놓았고
		개발자는 자바의 DBMS에게만 명령을 전달 하도록 해 놓았다.
		( ==> 개발자 입장에서 한가지 방식으로 처리하도록 된 것이다.)
		자바 안에 있는 DBMS가 실제 데이터베이스와 연결해서
		처리하도록 해 놓았다.
		
		이렇게 문제 해결을 위한 자바내에 만들어진 DBMS가 JDBC 이다.
		
		우리가 JDBC를 배운 다는 것은
		결국은 JDBC에게만 명령을 보내면(한가지 방식으로만 명령하면)
		JDBC가 알아서 필요한 데이터베이스의 DBMS에게 
		명령을 전달하게 된다.
		
		참고 ]
			JDBC와 실제 DBMS를 연결 하는 프로그램은
...
자바 개발자(데이터베이스 처리를 원하는)가 가장 먼저 해야 할 일은
	...
	2. 찾아낸 외부 API를 이클립스(IDE)에 등록해서 이클립스에서
	...
	public Test02_JDBC() {
		...
		try {
			...
			/*
				...
				JDBC에서는 오라클에서 접속을 의미하는 Session을 Connection 이라고 부른다.
