...
/*
	...
	==> 많은 양의 데이터를 손쉽게 보관할 수 있는 클래스들을 부르는 단어
				...
				특징 ]
					1. 입력순서를 보장하지 않는다.
						( 자신만의 규칙을 이용해서 데이터를 정렬해서 보관한다.
							처리속도는 빠르다.)
					2. 중복데이터가 입력이 되면 먼저 저장된 데이터를 삭제한 후
						저장한다.
						==> 중복데이터를 허용하지 않는다. 중복데이터는 한번만 저장한다.
			
			------------------------------------------------------------------------------------
			3. MAP 계열
				특징 ]
					1. 데이터를 그 데이터를 구분하는 "키값"과 한쌍으로 만들어서 저장한다.
					2.입력순서 보장하지 않는다.
					3. 같은 키값으로 데이터가 입력되면
						먼저 입력된 데이터는 삭제된다.
		...
		정리 ]
			...
			MAP계열은 위 두계열과는 다른 상위클래스를 가진다.
			==>
				LIST와 SET 은 다형성 처리가 가능하지만
				MAP은 안된다.
				MAP은 상위클래스가 다르므로 LIST나 SET으로 형변환이 불가능하다.
...
-------------------------------------------------------------------------------------------------
	...
	1. LIST 계열
		...
		1) Vector
			==> 내부적으로 배열의 형태를 만들어서 데이터를 보관하는 컬렉션의 일종
			...
			생성방법 ]
				Vector()
					==> 배열의 크기를 10으로 정한상태로 벡터를 만들어 준다.
						11번째 데이터가 입력되는 순간 크기를 20으로 변경을 하고
				...
				참고 ]
					컬렉션은 데이터의 양에 관계없이 언제나 데이터를 입력받을 수 있다.
					이때 데이터를 입력받을 수 있는 공간은 환보되어야 하는데
					공간을 확보하는 규칙은 
						현재공간 * 2
					의 형태로 공간을 확보하게된다.
		...
		2) ArrayList
			==> Vector의 신버젼으로
				...
				요즘은 Vector 보다는 ArrayList를 선호하는 경향이 있다.
	...
	참고 ]
		...
		==> 컬렉션은 많은 클래스들의 집합이고 형태도 모두 다르다.
			...
			이때 데이터를 꺼내는 방식은 서로 다를 수 있다.
			List 계열에서 꺼내는 함수를 Set계열에서는 사용할 수 없는 것이 그 예이다.
			==> 데이터에 따라서 꺼내는 방식이 달라질 수 있다는 말이다.
			
			이런 불편함을 해결하기 위해서 만들어진 클래스가
			Iterator 이다.
			List계열이던 Set계열이든 Iterator 타입으로만 만들어주면
			꺼내는 함수는 Iterator 멤버함수로 꺼내면 된다.
			
			따라서 List계열과 Set계열에 모두 iterator()함수가 있고
			이 함수의 반환값은 Iterator 타입이 된다.
			
			참고로 Iterator도  StringTokenize와 마찬가지로 
			버퍼메모리에 기억되고 순차적으로 꺼내고
...
public class Test01_Collection {
	...
	public Test01_Collection() {
		...
		for(int i = 0 ; i < vec.size() ; i++ ) {
			...
			// capacity() 	: 자장할 수 있는 공간의 수를 반환해주는 함수
			Object o = vec.get(i);
			
			System.out.println((int) o );
		}
		
		System.out.println(vec); // 오버라이드된 toString()가 자동 호출
	}
