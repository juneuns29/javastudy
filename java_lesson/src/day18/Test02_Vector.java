...
/*
	package와 import
		...
		==> 자바프로그램 소스코드가 들어있는 폴더를
			...
			이런 문제를 해결하기 위해서 만들어 놓은 한가지 규칙이 바로 패키지 이다.
			
			소스를 만든 사람이 그 프로그램이 포함된 폴더이름까지 정해서 프로그램을 만들도록 해 놓았다.
			
			결론적으로
				자바로 프로그램을 만들때는 반드시 이 프로그램이 어떤 폴더에 저장되는지를 알려줘야 한다.
			...
			참고 ]
				...
				주의 ]
					이때 사용가능한 클래스는 해당 패키지 바로 하위에 있는 클래스만 가능하다.
						...
						만약 Ab, Cd 클래스를 사용하려면
							import a.ab.*;
			참고 ]
				임포트 하지 않아도 클래스이름 앞에 경로를 밝혀주면 해당 클래스를 사용할 수 있다.
...
//		double pi = PI;
		...
		// 벡터와 어레이리스트는 형제관계이기 때문에 강제로 형변환은 가능하지만
		// 상속관계가 아니기때문에 사용할 때 문제가 생길 수 있다.
