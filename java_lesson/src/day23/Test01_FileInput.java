 ...
 */
	public Test01_FileInput() {
		...
		try {
			...
			/*
			// 1) 한글자만 읽어보자.
			int ch = fin.read(); // 이 함수는 반환값이 읽은 문자의 아스키코드를 반환해준다.
			// 출력
			...
			*/
			...
			int len = fin.read(buff); // 읽은 데이터 갯수를 반환해준다. 이때 읽은 데이터는 buff 배열에 기억된다.
			...
			*/
			...
			while(true) {
				...
				if(len == -1) {
					// read() 읽은 데이터가 없는 경우 -1을 반환해준다. 
					// 이 경우는 읽은 데이터가 없는 경우이므로
					// 읽기 작업이 모두 끝난 후이다.
					// 따라서 반복 작업을 종료해준다.
					break;
				}
				...
				// 함수 자체적으로 줄바꿈 기능이 있기 때문에 
				// 문서의 내용과는 무관하게 줄바꿈이 될 것이기 때문에...
			}
