...
/*
	PrintStream
		...
		기능
			1. 내부적으로 BufferdOutputStream과 연결해 놓았다.
			2. 사용자 편의를 위해서 자바의 데이터타입을 그대로 출력할 수 있도록 해 놓았다.
			3. 파일과 직업 연결하도록 해놓았다.
				==> 기본 스트림을 준비하지 않아도 사용할 수 있는 스트림이다.
					<== 내부적으로 기본스트림이 만들어져서 사용되게 된다.
					
		단점 ]
			한쌍을 이루는 입력계열 스트림이 없다.
 */
	public Test07_Print() {
		// 시 한편을 저장해보자.
		// 스트림 준비
		...
		try {
			// 스트림 만들고
			ps = new PrintStream("src/day23/진달래.txt");
			// 이 스트림은 파일에 연결된 보조스트림이다.
