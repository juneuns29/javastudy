...
/*
	Char 단위 스트림
	==> 한번에 입출력할 때 2바이트 단위로 입출력할 수 있는 스트림
		
		주의 ]
			기계어 상태의 내용은 절대로 문자단위로 입출력을 하면 안된다.
			이 스트림은 반드시 텍스트 문서에 한해서만 사용해야 한다.
			
			기계어 문서는 반드시 byte 단위 스트림을 이용해서 입출력해야 한다.
			
				예 ]
					가 		ㄱ : 0011		ㅏ : 0101
					라고 가정하면
					00110101 처럼 문자가 만들어질것으로 예상하겠지만
		...
		1. FileWrite / FileReader
			...
			내부적으로 char단위로(2byte 씩)처리 된다는 것만 달라질 뿐
			바이트단위 스트림과 사용법이 다를것이 없다.
			
		2. 관련된 필터(보조)스트림
			
			1) BufferedReader / BufferedWriter
				==> 중간에 버퍼 기능을 추가해서 스트림의 성능을 향상시키는 보조스트림
				
				핵심함수 ]
					char 단위의 기본 함수에는 문자열을 읽을 수 있는 함수가 없다.
					
					readLine()	==> 한줄 단위로 문자열을 그대로 읽어서 사용할 수 잇는 함수
					
					참고 ]
						사용상 주의사항 ]
							이 함수는 줄 단위로 한줄씩 읽는 함수이다.
							(\r\n(줄바꿈기호)를 발견하면 그 곳에서 읽는 것을 멈춘다는 것을 의미한다.)
							
							따라서 필요하면 줄단위를 강제로 입력해줘야 한다.
							참고로 이 함수는 줄바꿈기호를 읽지 않는다. 
							줄바꿈기호를 삭제한다 라는 의미
			...
			2) PrintWriter
				==> byte 단위의 PrintStream 처럼
					필터계열의 스트림이다.
						1. 버퍼의 기능을 가지고 있다.
						2. 직접 파일에 연결할 수 있다.(내부적으로 기본스트림을 만들어서 연결해 놓는다.)
						3. 사용자의 편의를 제공한다.
							==> 자바의 데이터형을 그대로 출력할 수 있다.
						4. byte단위의 스트림과도 연결 할 수 있다.
							
							
						참고 ]
							스트림 사용상 주의사항
								1. 반드시 같은 방향끼리만 연결할 수 있다.
								2. 반드시 같은 크기로만 연결 할 수 있다.
		...
		참고 스트림
			...
			예 ]
				FileInputStream fin = new FileInputStream("????");
				InputStreamReade tmp = new InputStreamReader(fin);
				BufferedReader br = new BufferedReader(tmp);
				
			참고 ]
				기본(타겟)스트림이 문자단위이고 보조스트림이 바이트단위인 경우에는 연결 할 수 없다.
				*****
...
-------------------------------------------------------------------------------------------------------------
	...
	객체의 직렬화
	==> 데이터는 자바의 기본형 이외에도 클래스 전체를 데이터로 외부장치와 입출력할 수 있다.
		==> 이때 함수는 제외되고
			...
			<==	클래스에 있는 변수의 내용이 순서대로 입출력이 되기때문에...
			
			그런데
			아무 클래스나 입출력되는 것은 아니고
			그 클래스가 반드시 직렬화 가능한 클래스여야 한다.
			직력화 가능한 클래스는 "Serializable" 인터페이스를 상속받아서 만들어진 클래스이다.
			
			직렬화는 변수의 내용만 입출력되므로
			네트워크 처럼 서로 떨어진 경우에는
			양쪽에 똑같은 클래스가 존재해야 된다.
			(왜냐하면 클래스 구조는 전달 되지 않고 변수의 내용만 전달 되기 때문에
