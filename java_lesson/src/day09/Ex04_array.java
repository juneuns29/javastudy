 ...
 */
	public static void main(String[] args) {
		...
		/*
		int kor1 = (int)(Math.random()*20 + 1);
		int eng1 = (int)(Math.random()*20 + 1);
		int math1 = (int)(Math.random()*20 + 1);
		...
		int kor2 = (int)(Math.random()*20 + 1);
		int eng2 = (int)(Math.random()*20 + 1);
		int math2 = (int)(Math.random()*20 + 1);
		...
		*/
		...
		for(int i = 0 ; i < 5 ; i++ ) {
			// 과목 데이터 입력
			for(int j = 0 ; j < 3; j++ ) { // 3 == a_ban[i].length - 1
				// 각 과목 점수 입력
				a_ban[i][j] = (int)(Math.random()*41 + 60); 
				// 41 <== 100 - 60 + 1 : (큰수 - 작은수 + 1) 
				// 총점 누적
				a_ban[i][3] += a_ban[i][j];
			}
		...
		for(int i = 0 ; i < 5 ; i++ ) {
			for(int j = 0 ; j < 5 ; j++ ) {
				System.out.print(a_ban[i][j] + ", "); // 각과목 꺼내서 출력
			}
			System.out.println(); // 줄바꿈
		}
