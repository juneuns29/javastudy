...
public class Circle {
	...
	/*
		...
		참고 ]
			함수 : 프로그램 제작의 최소단위
					특정 기능을 코드화 한 것
					엄밀히 따지면 자바에서의 함수는 Method라 불러하지만
					자바에서는 클래스 밖에 Method가 존재할 수 없으므로
					저는 함수라 부릅니다.
			...
			***
			생성자 함수
				:	클래스가 실행되기 위해서는 
					...
					Heap 영역에 기록되고
					이때 기록된 것의 주소를 모르면 사용할 수 없게된다.
					사용하려면 만들어진 것(Instance)의 주소를 기억해야 하는데
					기억하는 것을 담당하는 것이 변수(Reference)이다.
					
					그런데 Heap 영역에 올려지는 것들은 
					자동 초기화가 된다.
					기본데이터타입의 변수들은 모두 0, 0.0, false 로 초기화 되고
					참조형 타입의 경우는 null 로 초기화 된다.
					
					그런데 지금 이 클래스처럼
					객체가 만들어지는 순간 특정한 작업이 이미 처리되어야 하는 경우도 있다.
					이 클래스의 예를 들자면 반지름 셋팅과 같은 작업....
					
					이처럼 클래스가 객체가 되는 순간 해줘야 하는 작업들을 실행해주는 함수가
					생성자 함수이다.
